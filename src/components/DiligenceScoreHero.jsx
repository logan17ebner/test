...
export default function DiligenceScoreHero({ companyName, recommendation, diligenceScore }) {
  ...
  const barColor = diligenceScore >= 7 ? '#22C55E' : diligenceScore >= 4 ? '#F59E0B' : '#EF4444';
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl px-6 py-6 flex flex-col sm:flex-row sm:items-center gap-5">
      ...
      <div className="flex items-center gap-6 flex-shrink-0">
        ...
        <div className="text-center">
          <p className="text-slate-400 text-xs mb-1">Diligence Score</p>
          <p className="font-mono-data text-4xl font-bold text-slate-900 leading-none">
            {displayScore.toFixed(1)}
            <span className="text-slate-400 text-base font-normal ml-0.5">/10</span>
          </p>
          <div className="mt-2 h-1.5 w-24 bg-slate-100 rounded-full overflow-hidden">
            <div
              className="h-full rounded-full transition-all duration-1000 ease-out"
              style={{ width: `${diligenceScore * 10}%`, backgroundColor: barColor }}
            />
          </div>
        </div>
      </div>
